...
def train(args, train_dataset, model, tokenizer, fh, pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        for step, batch in enumerate(train_dataloader):
            inputs, labels, poss = batch
            batch_max_length = inputs.ne(tokenizer.pad_token_id).sum(-1).max().item()
            inputs = inputs[:, :batch_max_length]
            inputs = inputs.to(args.device)
            labels = labels.to(args.device)
            poss = poss.to(args.device)
            model.train()
            outputs = model(inputs, attention_mask=inputs.ne(tokenizer.pad_token_id), output_attentions=True)
            logits, attns = outputs[0], outputs[-1][-1]
            loss1 = celoss(logits, labels)
            if args.reghead == -1:
                attns = attns.mean(1)[:, 0, :]
            else:
                attns = attns[:, args.reghead, 0, :]
            # print(attns.shape)
            loss2 = -torch.sum(torch.log(attns + 1e-8) * poss[:, :attns.shape[1]]) / attns.shape[0]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        output_dir = os.path.join(args.output_dir, '{}-{}-{}'.format(checkpoint_prefix, global_step, round(results['accuracy'],4)))
                    else:
...
def evaluate(args, model, tokenizer, prefix="", eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            outputs = model(inputs, attention_mask=inputs.ne(tokenizer.pad_token_id))
            logits = outputs[0]
...
def main():
    ...
    parser.add_argument('--reghead', type=int, default=-1,
                        help="regularization head number")
    parser.add_argument('--lambd', type=float, default=1.,
                        help="regularization weight for attention heads")
    parser.add_argument('--prob', type=float, default=1.,
                        help="semi-supervised percentage")
    ...
    if pretrained:
        ...
        if args.model_type == "rnn":
            ...
            model = model_class.from_pretrained(pretrained, num_labels=2)
            model.resize_token_embeddings(len(tokenizer))
    else:
